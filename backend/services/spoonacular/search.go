...
func SearchRecipes(query string, number int) (*SpoonacularResponse, error) {
	...
	params.Add("addRecipeInformation", "true") // supaya dapat ID yang valid
	...
	for _, item := range rawResult.Results {
		nutrition, err := GetRecipeNutrition(item.ID)
		if err != nil {
			nutrition = Nutrition{} // biarkan kosong jika gagal
		}

		// Konversi kalori string (misal: "400 kcal") ke int
		var calories int
		fmt.Sscanf(nutrition.Calories, "%d", &calories)
